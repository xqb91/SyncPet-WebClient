<%-- 
    Document   : paso3
    Created on : 04-07-2016, 20:41:24
    ...
    <div class="container">
        ...
        <div class="row">
                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <table class="table table-bordered">
                            <tr>
                                <td>Nombre</td>
                                <td><c:out value="${mascota.getPropietario().getNombres()}"></c:out> <c:out value="${mascota.getPropietario().getApaterno()}"></c:out> <c:out value="${mascota.getPropietario().getAmaterno()}"></c:out></td>
                            </tr>
                            <tr>
                                <td>Dirección</td>
                                <td><c:out value="${mascota.getPropietario().getDireccion()}"></c:out></td>
                            </tr>
                            <tr>
                                <td>Comuna</td>
                                <td><c:out value="${mascota.getPropietario().getComuna().getNombre()}"></c:out></td>
                            </tr>
                        </table>
                    </div>
                </div>
            ...
            <div class="col-md-4">
                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h3 class="panel-title"><span class="glyphicon glyphicon-ok-sign" aria-hidden="true"></span> Información del Paciente</h3>
                    </div>
                    <div class="panel-body">
                        <table class="table table-bordered">
                            <tr>
                                <td>Nombre</td>
                                <td><c:out value="${mascota.getNombre()}"></c:out></td>
                            </tr>
                            <tr>
                                <td>Raza</td>
                                <td><c:out value="${mascota.getRaza().getNombre()}"></c:out> </td>
                            </tr>
                            <tr>
                                <td># Chip</td>
                                <td><c:out value="${mascota.getNumeroChip()}"></c:out></td>
                            </tr>
                        </table>
                    </div>
                </div>
            ...
            <div class="col-md-4">
                ...
                <div class="panel panel-default">
                    <div class="panel-heading">
                        <h3 class="panel-title"><span class="glyphicon glyphicon-calendar" aria-hidden="true"></span> Seleccione hora a reservar</h3>
                    </div>
                    <div class="panel-body">
                        <form action="registrarTomaHora" method="post">
                            <div class="form-group">
                                <label for="cmbVeterinario">Veterinario</label>
                                <select class="form-control" name="cmbVeterinario">
                                    <c:forEach var="ma" items="${listadoMedicos}">
                                        <c:if test="${ma.getVeterinario() != null}">
                                            <option value="<c:out value="${ma.getVeterinario().getIdVeterinario()}"></c:out>"><c:out value="Dr. ${ma.getVeterinario().getNombres()}"></c:out> <c:out value="${ma.getVeterinario().getApaterno()}"></c:out> <c:out value="${ma.getVeterinario().getAmaterno()}"></c:out> (<c:out value="${ma.getVeterinario().getEspecialidad()}"></c:out>)</option>
                                        </c:if>
                                    </c:forEach>
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                <label for="cmbHoraAtencion">Hora Atención</label>
                                <select class="form-control" name="cmbHoraAtencion">
                                    <c:forEach var="ma" items="${listadoHoras}">
                                        <option value="<c:out value="${ma}"></c:out>"><c:out value="${ma}"></c:out></option>
                                    </c:forEach>
                                </select>
                            </div>
                            
                            <div class="form-group">
                                <label for="cmbFecha">Hora Atención</label>
                                <input type="date" id="fecha" name="cmbFecha" min="<fmt:formatDate pattern="yyyy-MM-dd" value="${fechaHoy}" />"  class="form-control" />
                            </div>
                            <input type="hidden" name="txtMascota" value="${mascota.getIdMascota()}" />
                            <input type="hidden" name="txtSucursal" value="${mascota.getPropietario().getSucursal().getIdSucursal()}" />
                            <button type="submit" class="btn btn-default btn-sm">Tomar Hora</button>
                        </form>
                    </div>
                </div>
            </div>
